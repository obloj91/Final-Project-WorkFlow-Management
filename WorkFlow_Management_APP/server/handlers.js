...
findUser ? res.status(200).json({ status: 200, data: findUser, message: "User found!" })
   : res.status(200).json({ status: 404, data: singleUserID, message: "User cannot be found!" })
...
if(!firstName || !lastName || !username || (!email || email.includes("@")=== false) || !password){
  return res.status(200).json({status: 400, data: newUser, message: "Missing-data!"}) //400 is BAD REQUEST
}
//check to see if user name or email account already exists in the database
else if(users.find((user) => user.email===email || user.lastName === lastName || user.firstName === firstName)){
  return res.status(200).json({status: 400, data:newUser, message: "Account already exists!"})
}
//check to see if chosen Username already is picken by another user in our existing database
else if(users.find((user) => user.username===username)){
  return res.status(200).json({status: 400, data: username, message: "Username already taken, please choose another one!"})
}
// if everything checks out we can add the new user account information in our users database collection
else{
  await db.collection("users").insertOne(newUser);
  return res.status(200).json({status: 201, message: "New User CREATED!", data: newUser})
}} 
catch(error){
    console.log(error.stack);
    client.close();
...
findAdmin? res.status(200).json({ status: 200, data: findAdmin, message: "Administrator account found!" })
   : res.status(200).json({ status: 404, data: findAdmin, message: "Administrator account cannot be found!" })
...
findProject ? res.status(200).json({ status: 200, data: findProject, message: "Project found!" })
   : res.status(200).json({ status: 404, data: singleProjectID, message: "Project cannot be found!" })
...
if(!projectName || !projectType || !projectDeadline){
  return res.status(200).json({status: 400, data: newProject, message: "BAD REQUEST: missing keys or values!"})
}
...
// if everything checks out we can add the new PROJECT in our PROJECTS database collection
  ...
  return res.status(200).json({status: 201, message: "New Project CREATED!", data: newProject})
}} 
catch(error){
    console.log(error.stack);
    client.close();
...
return res.status(200).json({ status: 200, data: tasks, message: "Success!" });
...
findTask ? res.status(200).json({ status: 200, data: findTask, message: "Task found!" })
   : res.status(200).json({ status: 404, data: singleTaskID, message: "Task cannot be found!" })
...
const addTask = async(req,res)=>{
  ...
  const{
    taskName, projectName, dueDate, taskID,
    assignedUsers, details, nbrDeliverables, 
    status, comments} = newTask; //deconstructing Task JSON object
...
if(!projectName || !taskName || !dueDate || !details || !nbrDeliverables || assignedUsers.length < 1 ){
  return res.status(200).json({status: 400, data: newTask, message: "BAD REQUEST: missing keys or values!"})
}
...
// we add the new task to our tasks database collection
  ...
  const updatedTasksIDArray = await db.collection("projects").updateOne({projectName: projectName}, //query 
  ...
  return res.status(200).json({status: 201, message: "New Task CREATED!", data: newTask, data2: updatedTasksIDArray})
  //} 
  //else{
...
const updateTask = async(req,res)=>{
  ...
  const{
    taskName, projectName, dueDate, taskID,
    assignedUsers, details, nbrDeliverables, status, delivrablesPathsArr} = existingTask; //deconstructing Task JSON object
  ...
  if(findTask === undefined) {
    return res.status(200).json({ status: 404, data: existingTask, message: "Task cannot be found!" })
   }
...
{ dueDate: req.body.dueDate, 
 ...
 const updatedTask = await db.collection("tasks").updateOne(query, newValues);

 res.status(200).json({ status: 200, data: updatedTask, message: "Task updated successfully!" })
}} 
catch(error){
    console.log(error.stack);
    client.close();
...
if (findTask === undefined){
  return res.status(200).json({status: 404, data: singleTaskID, message: "Task cannot be found!" })
}
else{
...
const deleteTaskIDFromProjects = await db.collection("projects").updateOne({projectName: findTask.projectName}, //query 
{$pull:{tasks: singleTaskID}});   //delete taskID values

//delete the task from tasks database collection
const deleteTask = await db.collection("tasks").deleteOne({taskID: singleTaskID}); 

  res.status(200).json({ status: 200, data: deleteTask, data2: deleteTaskIDFromProjects, message: "Task deleted successfully!" })
}
...
const fileStorageEngine= multer.diskStorage({
  // cb is the callback function 
    destination: (req, files, cb ) => {
      cb (null, "C:/Users/Alan/Documents/2021_PROGRAMMING/Final Project/WorkFlow_Management_APP/server/Uploads")
    },
...
module.exports = {
  ...
  addUser,getProjects,
  getSingleProject, addProject,
  getTasks,getSingleTask, addTask,
  updateTask,deleteTask, getAdmin, upload, uploadFile,uploadFiles
